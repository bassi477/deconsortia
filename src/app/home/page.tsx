...
function HomeJumbotron() {
  ...
  React.useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentImageIndex((currentValue) =>
        currentValue === maxImageIndex ? 0 : currentValue + 1
      );
      setCurrentTextIndex((currentValue) =>
        currentValue === maxTextIndex ? 0 : currentValue + 1
      );
    }, intervalTime);
  ...
  return (
    <div
      className="relative overflow-hidden bg-cover bg-no-repeat p-12 rounded-b-lg h-[70vh]"
      style={{
    ...
    >
      ...
      >
        <div className="flex h-full items-center justify-center p-6 xl:p-12">
          <div className="text-white">
            <h2 className="mb-4 text-sm sm:text-2xl md:text-4xl xl:text-5xl 2xl:text-7xl font-semibold">
              {HomeSlideshowTextList[currentTextIndex]}
            </h2>
          </div>
        </div>
      </div>
...
function HomePeopleSectionItem(props: HomePeopleSectionItemProps) {
  ...
  return (
    <div key={props.key} className="mb-12 lg:mb-16 xl:mb-32">
      ...
      />
      <h5 className="mb-4 text-md md:text-lg xl:text-2xl 2xl:text-4xl font-bold">
        {props.member.name}
      </h5>
    </div>
  );
